        ...
        if closes[i] < opens[i] and future_change > threshold:
            ...
            for j in range(i + 1, len(df)):
                if lows[j] <= bottom:
                    ...
                    break
            print(f"Bullish OB at {times[i]} (bottom={bottom}): Mitigated={mitigated} by {mitigating_candle}")
        ...
        if closes[i] > opens[i] and future_change < -threshold:
            ...
            for j in range(i + 1, len(df)):
                if highs[j] >= top:
                    ...
                    break
            print(f"Bearish OB at {times[i]} (top={top}): Mitigated={mitigated} by {mitigating_candle}")
